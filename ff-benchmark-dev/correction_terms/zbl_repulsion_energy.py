...
class ZBLRepulsionEnergy(nn.Module):
    ...
    def forward(
        ...
        z = Zf ** F.softplus(self._apow.to(device))
        a = (z[idx_i] + z[idx_j]) * F.softplus(self._adiv.to(device))
        a1 = F.softplus(self._a1.to(device)) * a
        a2 = F.softplus(self._a2.to(device)) * a
        a3 = F.softplus(self._a3.to(device)) * a
        a4 = F.softplus(self._a4.to(device)) * a
        c1 = F.softplus(self._c1.to(device))
        c2 = F.softplus(self._c2.to(device))
        c3 = F.softplus(self._c3.to(device))
        c4 = F.softplus(self._c4.to(device))

...
class D4DispersionEnergy(nn.Module):
    def __init__(
        ...
        self.register_buffer(
            ...
            torch.load(os.path.join(directory, "refsys.pth"), map_location=device)[:Zmax],
        )
        self.register_buffer(
            "zeff", torch.load(os.path.join(directory, "zeff.pth"), map_location=device)[:Zmax]  # [Zmax]
        )
        self.register_buffer(
            "refh",  # [Zmax,max_nref]
            torch.load(os.path.join(directory, "refh.pth"), map_location=device)[:Zmax],
        )
        self.register_buffer(
            "sscale", torch.load(os.path.join(directory, "sscale.pth"), map_location=device)  # [18]
        )
        self.register_buffer(
            "secaiw", torch.load(os.path.join(directory, "secaiw.pth"), map_location=device)  # [18,23]
        )
        self.register_buffer(
            "gam", torch.load(os.path.join(directory, "gam.pth"), map_location=device)[:Zmax]  # [Zmax]
        )
        self.register_buffer(
            "ascale",  # [Zmax,max_nref]
            torch.load(os.path.join(directory, "ascale.pth"), map_location=device)[:Zmax],
        )
        self.register_buffer(
            "alphaiw",  # [Zmax,max_nref,23]
            torch.load(os.path.join(directory, "alphaiw.pth"), map_location=device)[:Zmax],
        )
        self.register_buffer(
            "hcount",  # [Zmax,max_nref]
            torch.load(os.path.join(directory, "hcount.pth"), map_location=device)[:Zmax],
        )
        self.register_buffer(
            "casimir_polder_weights",  # [23]
            torch.load(os.path.join(directory, "casimir_polder_weights.pth"), map_location=device)[:Zmax],
        )
        self.register_buffer(
            "rcov", torch.load(os.path.join(directory, "rcov.pth"), map_location=device)[:Zmax]  # [Zmax]
        )
        self.register_buffer(
            "en", torch.load(os.path.join(directory, "en.pth"), map_location=device)[:Zmax]  # [Zmax]
        )
        self.register_buffer(
            "ncount_mask",  # [Zmax,max_nref,max_ncount]
            torch.load(os.path.join(directory, "ncount_mask.pth"), map_location=device)[:Zmax],
        )
        self.register_buffer(
            "ncount_weight",  # [Zmax,max_nref,max_ncount]
            torch.load(os.path.join(directory, "ncount_weight.pth"), map_location=device)[:Zmax],
        )
        self.register_buffer(
            "cn",  # [Zmax,max_nref,max_ncount]
            torch.load(os.path.join(directory, "cn.pth"), map_location=device)[:Zmax],
        )
        self.register_buffer(
            "fixgweights",  # [Zmax,max_nref]
            torch.load(os.path.join(directory, "fixgweights.pth"), map_location=device)[:Zmax],
        )
        self.register_buffer(
            "refq",  # [Zmax,max_nref]
            torch.load(os.path.join(directory, "refq.pth"), map_location=device)[:Zmax],
        )
        self.register_buffer(
            "sqrt_r4r2",  # [Zmax]
            torch.load(os.path.join(directory, "sqrt_r4r2.pth"), map_location=device)[:Zmax],
        )
        self.register_buffer(
            "alpha",  # [Zmax,max_nref,23]
            torch.load(os.path.join(directory, "alpha.pth"), map_location=device)[:Zmax],
        )
    ...
    def forward(
        ...
        Z = Z.long() # Convert Z to long tensor
        Zi = Z[idx_i]
        Zj = Z[idx_j]
        ...
        covcn = rij.new_zeros(N)
        tmp = tmp.to(dtype=covcn.dtype)  # Convert 'tmp' to the same dtype as 'covcn'
        covcn.index_add_(0, idx_i, tmp)
        ...
        edisp = rij.new_zeros(N)
        pairwise = pairwise.to(dtype=edisp.dtype)  # Convert 'pariwise' to the same dtype as 'edisp'
        edisp.index_add_(0, idx_i, pairwise)
